...
const { categories, authors, articles, global, about, slides, realTimeUpdates } = require('../data/data.json');
...
async function importSlides() {
  for (const slide of slides) {
    const image = await checkFileExistsBeforeUpload([slide.imageUrl]);
...
async function importRealTimeUpdates() {
  for (const update of realTimeUpdates) {
    await createEntry({
      model: 'real-time-update',
      entry: {
        ...update,
        // Make sure it's not a draft
        publishedAt: Date.now(),
